...
router.post('/claim', async (req, res) => {
	try {
		const { sessionId, name, country, city, socialLinks } = req.body;
		...
		if (checkout.formCompleted) {
			return res
				.status(400)
				.json({ error: 'This session has already been claimed' });
		}

		console.log(checkout);
		const normalizedEmail = checkout.email.toLowerCase().trim();

		const existingBeliever = await Leaderboard.findOne({
		...
		if (existingBeliever) {
			return res.status(400).json({
				error:
					'This email already has a believer profile. Please log in instead.',
			});
		}

		console.log(existingBeliever);
